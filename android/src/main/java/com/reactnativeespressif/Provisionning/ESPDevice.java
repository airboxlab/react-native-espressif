...
import com.reactnativeespressif.Provisionning.transport.BLETransport;
import com.reactnativeespressif.Provisionning.transport.SoftAPTransport;
import com.reactnativeespressif.Provisionning.transport.Transport;
import com.reactnativeespressif.Provisionning.utils.MessengeHelper;
...
public class ESPDevice {
    ...
    public ESPDevice(Context context, ESPConstants.TransportType transportType, ESPConstants.SecurityType securityType, EspressifConfig config) {
        ...
        switch (transportType) {
            ...
            case TRANSPORT_BLE:
                transport = new BLETransport(context, config.getBleConfigUuid());
                break;
    ...
    public void connectToDevice(Promise promise) {

        switch (transportType) {

            case TRANSPORT_BLE:
                ((BLETransport) transport).connect(bluetoothDevice, UUID.fromString(primaryServiceUuid), promise);
                break;
...
//    }
    ...
    public interface DeviceInfoListener {
      void Success(DeviceInfo.DeviceInfoResponse response);
      void Error(Exception exception);
    }

    public void getDeviceInfo(DeviceInfoListener listener){
      DeviceInfo.DeviceInfoRequest info = DeviceInfo.DeviceInfoRequest.newBuilder().setProtocolVersion(1).build();

      session.sendDataToDevice("device-info", info.toByteArray(), new ResponseListener() {
        @Override
        public void onSuccess(byte[] returnData) {
          try {
            DeviceInfo.DeviceInfoResponse response = DeviceInfo.DeviceInfoResponse.parseFrom(returnData);
